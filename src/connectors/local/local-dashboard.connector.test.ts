...
describe('local dashboard persistence', () => {
  it('restores saved tasks and time in a new connector instance', async () => {
    ...
    const state = { ...createInitialState(), completedTaskIds: ['update'], elapsedSeconds: 3610 };
  ...
  it('migrates the previous HTML replica state and removes the legacy entry on save', async () => {
    const storage = createSafeStorage();
    storage.setItem(
    ...
    expect(state.completedTaskIds).toEqual(['policy']);
    expect(state.elapsedSeconds).toBe(400);
    expect(state.preferences.reduceMotion).toBe(true);
    await connector.persistState(state);
    expect(storage.getItem('crextio-dashboard-v1')).toBeNull();
  ...
  it('discards saved task ids that are no longer present in the data', async () => {
    ...
    storage.setItem(
      ...
      JSON.stringify({ ...createInitialState(), completedTaskIds: ['interview', 'removed-task'] }),
    );
  ...
  it('returns isolated snapshots and preserves preferences when resetting', async () => {
    ...
    first.state.completedTaskIds.push('update');
    expect((await connector.load()).data.employee.name).toBe('Lora Piterson');
